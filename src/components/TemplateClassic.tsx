...
export default function TemplateClassic({ data }: TemplateProps) {
  ...
  if (contact.email) {
    primaryContactItems.push(
      <a key="email" href={`mailto:${contact.email}`} className="text-black hover:underline">
        {contact.email}
      </a>
    );
  }
  if (contact.websiteUrl) {
    primaryContactItems.push(
      <a key="web" href={contact.websiteUrl} target="_blank" rel="noreferrer" className="text-black hover:underline">
  ...
  const sectionRenderers = {
    summary: () => displaySummary ? (
      <ResumeSection title="PROFESSIONAL SUMMARY" style={getSectionStyle('summary', data.customization)} titleClassName="text-[11pt] font-bold uppercase tracking-widest text-black border-b border-black pb-1">
        <p className="text-[10pt] leading-relaxed pt-1">{displaySummary}</p>
      </ResumeSection>
    ) : null,
              ...
              <EntryHeader
                left={
                  <div>
                    ...
                    {exp.location && <span className="text-gray-600 text-[9.5pt] ml-2">({exp.location})</span>}
                  </div>
                }
          ...
          {education.map((edu) => (
            <div key={edu.id} className="space-y-0.5">
              <EntryHeader
                ...
                rightClassName="text-[9.5pt] text-gray-700 italic shrink-0 ml-2"
              />
              {edu.gpa && <p className="text-[9.5pt] text-gray-700">GPA: {edu.gpa}</p>}
        ...
        <div className="space-y-3 pt-1">
          {projects.map((proj) => (
            <div key={proj.id} className="space-y-1">
              <EntryHeader
                left={
                  <div className="flex items-center gap-2">
                    ...
                    {proj.link && (
                      <a href={proj.link} target="_blank" rel="noreferrer" className="text-[9pt] text-gray-800 underline">
                        [Demo]
                      </a>
                    )}
                    {proj.githubUrl && (
                      <a href={proj.githubUrl} target="_blank" rel="noreferrer" className="text-[9pt] text-gray-600 underline">
                        [Source]
                      </a>
                    )}
                  </div>
              ...
              {proj.tech.length > 0 && (
                <p className="text-[9pt] text-gray-700 italic">
    ...
    skills: () => ((skills && skills.length > 0) || (skillCategories && skillCategories.length > 0)) ? (
      <ResumeSection title="AREAS OF EXPERTISE" style={getSectionStyle('skills', data.customization)} titleClassName="text-[11pt] font-bold uppercase tracking-widest text-black border-b border-black pb-1">
        <div className="pt-1">
          {skillCategories && skillCategories.length > 0 ? (
            <div className="space-y-1 text-[10pt] text-gray-900">
              {skillCategories.map((cat) => (
                <div key={cat.id || cat.name}>
                  <strong className="text-black uppercase tracking-wider text-[9.5pt]">{cat.name}:</strong>{' '}
                  <span>{cat.skills.join(', ')}</span>
                </div>
              ))}
            </div>
          ) : (
            <p className="text-[10pt] text-gray-900">{skills.join(', ')}</p>
          )}
        </div>
      </ResumeSection>
    ) : null,
      ...
      <>
          ...
          <ResumeSection title="CERTIFICATIONS" style={getSectionStyle('certifications', data.customization)} titleClassName="text-[11pt] font-bold uppercase tracking-widest text-black border-b border-black pb-1">
            <div className="space-y-2 pt-1">
              {data.certifications.map((cert) => (
                <div key={cert.id} className="space-y-0.5 text-[10pt] text-gray-900">
                  <EntryHeader left={<strong className="text-black">{cert.title}</strong>} right={cert.date} rightClassName="text-[9.5pt] text-gray-700 italic shrink-0 ml-2" />
                  <div className="text-gray-800 text-[9.5pt] italic">{cert.subtitle}</div>
                  {cert.description && <p className="text-gray-700 text-[9.5pt] pt-0.5">{cert.description}</p>}
                </div>
              ))}
            </div>
          </ResumeSection>
          ...
          <ResumeSection title="HONORS & AWARDS" style={getSectionStyle('achievements', data.customization)} titleClassName="text-[11pt] font-bold uppercase tracking-widest text-black border-b border-black pb-1">
            <div className="space-y-2 pt-1">
              {achievements.map((ach) => (
                <div key={ach.id} className="flex justify-between text-[10pt] text-gray-900">
                  <span>
                    <strong className="text-black">{ach.title}</strong> — {ach.issuer}
                  </span>
                  <span className="text-gray-700 italic shrink-0">{ach.date}</span>
                </div>
              ))}
            </div>
          </ResumeSection>
          ...
          <ResumeSection title="PUBLICATIONS" style={getSectionStyle('publications', data.customization)} titleClassName="text-[11pt] font-bold uppercase tracking-widest text-black border-b border-black pb-1">
            <div className="space-y-2 pt-1">
              {data.publications.map((pub) => (
                <div key={pub.id} className="space-y-0.5 text-[10pt] text-gray-900">
                  <EntryHeader left={<strong className="text-black">{pub.title}</strong>} right={pub.date} rightClassName="text-[9.5pt] text-gray-700 italic shrink-0 ml-2" />
                  <div className="text-gray-800 text-[9.5pt] italic">{pub.subtitle}</div>
                  {pub.description && <p className="text-gray-700 text-[9.5pt] pt-0.5">{pub.description}</p>}
                </div>
              ))}
            </div>
          </ResumeSection>
        ...
        {data.customSections && data.customSections.length > 0 && data.customSections.map((cs) => (
          cs.items && cs.items.length > 0 && (
            <ResumeSection key={cs.id} title={cs.name ? cs.name.toUpperCase() : 'CUSTOM SECTION'} style={getSectionStyle('custom', data.customization)} titleClassName="text-[11pt] font-bold uppercase tracking-widest text-black border-b border-black pb-1">
              <div className="space-y-2 pt-1">
                {cs.items.map((item) => (
                  <div key={item.id} className="space-y-0.5 text-[10pt] text-gray-900">
                    <EntryHeader left={<strong className="text-black">{item.title}</strong>} right={item.date} rightClassName="text-[9.5pt] text-gray-700 italic shrink-0 ml-2" />
                    {item.subtitle && <div className="text-gray-800 text-[9.5pt] italic">{item.subtitle}</div>}
                    {item.description && <p className="text-gray-700 text-[9.5pt] pt-0.5">{item.description}</p>}
                  </div>
                ))}
              </div>
            </ResumeSection>
  ...
  return (
    <div className="font-serif text-black leading-normal">
      ...
      <header className="border-b-2 border-black pb-3 mb-5 text-center">
        ...
        {basicInfo.tagline && (
          <p className="text-[11pt] italic text-gray-800 mt-1">
            {basicInfo.tagline}
          </p>
        )}
        ...
        {primaryContactItems.length > 0 && (
          <div className="flex flex-wrap justify-center items-center gap-x-2.5 gap-y-1 text-[9.5pt] text-gray-800 mt-2 font-sans">
        ...
        {contact.socialLinks.length > 0 && (
          <div className="flex flex-wrap justify-center items-center gap-x-3 gap-y-1 text-[9.5pt] text-black font-semibold mt-1 font-sans">
