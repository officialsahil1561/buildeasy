      ...
      <div className="flex flex-col gap-6">
        ...
        {displaySummary && (
          <ResumeSection title="Summary" themeColor={themeColor} style={getSectionStyle('summary', data.customization)}>
            <p className="text-[10pt] text-gray-700 leading-relaxed">{displaySummary}</p>
          </ResumeSection>
        )}
                  ...
                  <EntryHeader
                    ...
                    right={
                      <div className="flex gap-3 text-[9.5pt]">
                        {proj.link && <a href={proj.link} target="_blank" rel="noreferrer" className="hover:underline font-semibold" style={{ color: themeColor }}>Live Demo ↗</a>}
                        {proj.githubUrl && <a href={proj.githubUrl} target="_blank" rel="noreferrer" className="hover:underline font-semibold" style={{ color: themeColor }}>GitHub ↗</a>}
                      </div>
                    }
                  />
        ...
        {((skillCategories && skillCategories.length > 0 && skillCategories.some(c => c.skills.length > 0)) || skills.length > 0) && (
          <ResumeSection title="Skills" themeColor={themeColor} style={getSectionStyle('skills', data.customization)}>
            <div className="space-y-1 pt-1 text-[10pt] text-gray-700">
        ...
        {achievements.length > 0 && (
          <ResumeSection title="Awards" themeColor={themeColor} style={getSectionStyle('achievements', data.customization)}>
            <div className="space-y-2">
        ...
        {data.customSections && data.customSections.length > 0 && data.customSections.map((cs, csIdx) => (
          cs.items && cs.items.length > 0 && (
            <ResumeSection key={cs.id || csIdx} title={cs.name || 'CUSTOM SECTION'} themeColor={themeColor} style={getSectionStyle('custom', data.customization)}>
              <div className="space-y-2">
